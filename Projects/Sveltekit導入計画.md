...
https://npmtrends.com/@sveltejs/kit
### 2.2 誕生の背景
...
1. **コンパイラである（The Compiler）**
   ...
   - Svelte: 開発者の書いたコードを、ビルド（コンパイル）の段階で「極限まで最適化された素のJavaScript」に変換する。
	結果、ユーザーのブラウザには余計なフレームワークコードが不要となり、ファイルサイズが圧倒的に小さく、動作も高速になる。
...
> - SvelteKit: 90%
>
> [State of JS 2024](https://2024.stateofjs.com/en-US/libraries/#tier_list)
...
将来を見据えると、**AIとの共同開発**や**仕様駆動開発（SDD: Spec-Driven Development / Specification-Driven Development）**が主流になると考えてます。
これらのAI開発手法では、仕様書や型定義からコードを生成するため、
**型情報が仕様書として機能するTypeScriptが不可欠**となる。
...
> 
> **結論**:
> 学習コストを理由にTypeScriptを採用しないことは、短期的には楽かもしれないが選択の幅を狭め、
> 長期的には技術的負債を増やすことになる。
> 特に、AIとの共同開発やモダンなライブラリの活用を考えると、TypeScriptの採用は必須と考えます。
...
これは単なる技術的な優位性だけではなく、リッチなライブラリを採用することで、複雑なUIの実装がかなり楽になる。

時間の短縮につながるため、他の作業に時間を割けるようになる。

これもまた、会社への貢献になる。
開発効率が上がれば、より多くの機能を実装でき、より良いサービスを提供できる。
...
全ての責任は私が負います。
コード品質の担保、維持も今後厳しくチェックしていきます。

フロントの処理なので、致命的な状況になる事は無いと考えてますが、
もしこれは駄目だという状況に陥った場合、休みの日にでも既存の作りに合わせて書き直す事を約束します。
