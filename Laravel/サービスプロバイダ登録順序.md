---
title: ServiceServiceProvider の登録順序の重要性
created: 2025-12-03
updated: 2025-12-03
category: Laravel
tags:
...
## Laravelの自動解決（Auto-resolution）

Laravelは型ヒントされた依存関係を自動解決します。ただし、以下のケースでは明示的な登録が必要です。

### 明示的な登録が必要なケース

1. インターフェースの実装を指定する場合

- 例：IReservationMailService → ReservationMailServiceProviderで登録済み

1. シングルトンとして管理したい場合

- 例：CacheService、LanguageService など

1. 循環参照を回避する場合

- 例：app()->make()を使う場合

1. コンストラクタで直接newを使っている場合（本来は修正すべき）

- 例：ReservationCommodityServiceがnew PaymentBusinessService()を使っている

### 現在のServiceServiceProviderの問題点

多くのサービスは自動解決可能ですが、一部のサービスがまだDIを使っていないため、明示的な登録が必要な状態です。
...
多くのサービスがまだnewで直接インスタンス化しています：

- ReservationCommodityService → new PaymentBusinessService()

- CourseStockCalendarService → new ReservationCommodityService()

- PlanService → new CancelPolicyService(), new BlobService(), etc.

## 結論

理論的には多くのサービスは自動解決可能ですが、現状では以下の理由で明示的な登録が必要です。

### 明示的な登録が必要な理由

1. シングルトンとして管理したい場合

- 例：CacheService、LanguageServiceなど

- 自動解決では毎回新しいインスタンスが作成される

1. インターフェースの実装を指定する場合

- 例：IReservationMailService → 別のServiceProviderで登録済み
...
1. シングルトンとして管理したいサービスだけを明示的に登録

2. それ以外は自動解決に任せる（DIを使うように修正後）

実際に、多くのサービスは自動解決できます。ただし、シングルトンとして管理したい場合は明示的な登録が必要です。

ServiceServiceProviderを簡素化して、シングルトンとして管理したいサービスだけを登録するように変更しますか？それとも、現状のまま（newを使っているサービスがあるため、明示的な登録が必要）にしておきますか？